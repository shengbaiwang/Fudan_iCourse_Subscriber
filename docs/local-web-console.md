...
其中四个本机分区为整理区、学习区、查阅区和归档区，新课程默认在整理区，可在
...
- **上传录音**：mp3/m4a/wav/flac/ogg/opus/mp4/mov（≤500MB）。录音在本机用
  ...
  记录），云端音频即删。上传请求本身只做校验+加密+暂存（秒回），dispatch
  失败会自动重试（8 次 × 30 秒）；若 workflow 文件尚未合并到 main 会明确
  提示先 push main（GitHub 对不存在的 workflow 只返回 404）。此外 manifest
  的 push 本身也会触发一次转写（幂等：转写已新于上传时直接跳过），因此
  即使 dispatch 偶发失败仍有兜底。同步轮询约 15 秒一次；云端无可用模型时
  只写回转写文本，可在本地手动生成笔记。
