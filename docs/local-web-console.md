...
详情页可勾选多个版本并排对比，并标记当前生效版本。当同一课堂（或讲座）存在
≥2 个笔记版本时，详情页会出现「盲测对比」：进入独立对比页后版本会随机匿名排布
（方案 A/B/C…），默认不显示生成模型；点击「揭示模型」才能看到对应模型与生成时间。
可先点「更喜欢这份」记录盲测偏好再揭示，也可「重新洗牌」再测一轮，或切换到
「公开对比」直接查看模型名。模型管理和
