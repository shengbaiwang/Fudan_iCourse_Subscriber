...
本地与 GitHub Pages 共用 `local_web/static/` 的界面源码，采用一致的“课程 → 课次 → 笔记详情”主路径：顶部导航提供课程、搜索、订阅和
...
详情页可勾选多个版本并排对比，并标记当前生效版本。详情页始终提供「盲测对比」入口：
进入独立对比页后，已有版本会随机匿名排布（方案 A/B/C…），默认不显示生成模型；点击
「揭示模型」才能看到对应模型与生成时间。可先点「更喜欢这份」记录盲测偏好再揭示，也可
「重新洗牌」再测一轮，或切换到「公开对比」直接查看模型名。版本不足 2 个时不必先去
重跑页——对比页内即可选择模型生成对照笔记（课次走 Actions 重跑；讲座需重新输入
API Key 本地生成），完成后「刷新版本」即可开始盲测。模型管理和
Obsidian 按需同步归入“设置”，因此日常阅读不会被管理项打断。本地入口通过本机 API 运行；Pages 使用浏览器数据适配层，无需本地服务。
Obsidian 同步和模型连接测试只在本地入口提供。

模型管理采用“左侧服务商列表、右侧当前配置”的布局。可以搜索服务商、启用或停用，并调整
调用优先级；模型使用逐项列表，支持搜索、输入模型 ID 添加、移除及上下排序。
API Key Secret 名称收在“高级设置”中。编辑后统一点击“保存到 GitHub”，切换服务商或
返回设置再进入会保留本次草稿；草稿不会写入浏览器存储，刷新或关闭页面前会提示未保存。
模型连接测试使用首选模型，需要重新输入 Key；保存成功后清空输入的 Key，已配置的 Secret 留空保留。
本地控制台可点击“获取模型”，使用本次输入的 Key 请求服务商的 `/models` 目录，搜索并逐项加入草稿；已添加的模型会标注。获取失败可重试，也可手动输入模型 ID。Pages 仍使用手动添加。
...
课程页左侧是分类侧栏（移动端为「课程」标题旁的抽屉按钮）：全部课程、星标，以及可折叠的
分区、学期、院系三个分组，每项带课程计数，选中即过滤右侧列表；选择与折叠状态保存在本机。
分区可自由命名、排序和删除，归档独立显示。新资料库默认未分区，旧资料库保留原分区，可在
课程卡片中直接移动。分区记录仅保存在本机 `course-zones.json`，不改变 GitHub 订阅、
Actions 或远端课程数据。学期与院系取自课程目录中该课程最近的学期。
Pages 的分区和笔记改名按仓库保存在浏览器中，
不会自动与本地偏好同步。
...
课程卡片、课次列表和笔记详情可打开“导出 / 删除”，按整门课程或所选课次触发工作流。
订阅页的“单次运行”可处理指定课程，且不改变订阅。Pages 的连接、部署和凭据说明见
[统一控制台](frontend-readme.md)。
...
界面依赖，之后启动服务和浏览器。如果默认的 `8765` 端口被其他程序占用，启动器会在
`8766`–`8785` 中选择空闲端口，并在 Terminal 中显示实际地址；再次双击会打开已运行的
控制台。Terminal 窗口保持打开表示本地服务正在运行。
...
顶栏「讲座」是独立于课程库的本机笔记区，有两种来源：

- **上传录音**：mp3/m4a/wav/flac/ogg/opus/mp4/mov（≤500MB）。录音在本机用
  课程同款 AES-256-CBC 信封加密后，由后台队列按约 4MB/片推到 Fork 的
  `talk-audio` 分支（每片独立重试 3 次，单片超时 120 秒；已传分片记在
  本机，可断点续传），全部完成后才写清单并触发 `talk_transcribe.yml`。
  该 workflow 与 `check.yml` 共用同一套 ffmpeg + sherpa-onnx（SenseVoice +
  Silero VAD）安装步骤，转写直接调用 `Transcriber.transcribe_video`，摘要
  直接调用 `Summarizer.summarize`（同一套供应商/模型回退链、同一份共用
  prompt、同一个 `# 标题` 拆分 + 兜底标题逻辑）——和 iCourse 课次走完全
  相同的“转写 → 摘要”链。转写文本与笔记一起加密写回
  `talk_transcripts/<id>.json.enc`，本地同步后自动落为 `ready`（含版本
  记录）。上传请求完成校验、加密和本地暂存后返回，上传与同步由服务后台执行，
  关闭网页不影响处理。工作流部署检查在分片上传前完成；临时网络错误有界重试，
  永久错误显示在讲座详情，可点击「继续处理」。工作流需要部署到仓库默认分支，
  由显式 dispatch 启动，每次请求独立关联对应运行；不依赖音频分支的 push 事件。
  每次转写完成会先保存加密检查点；摘要失败后重试复用转写，长文本分段生成并
  逐段保存进度。服务每 15 秒同步结果并检查工作流失败状态。笔记确认落库后清理
  本地上传暂存；云端加密音频与检查点保留供恢复，删除讲座时尝试一并清理（网络失败时远端密文可能仍保留）。
  无可用模型时保留转写与错误，也可在本地手动生成笔记。
- **粘贴转写**：已有转写文本时直接粘贴，选模型生成笔记（含 AI 标题与版本
  记录）。

讲座数据只保存在本机 `local_talks.db`，不上 `data` 分支、不进订阅/邮件/
Obsidian 同步。首次上传前需确保 Fork 已启用 Actions，且本地控制台 Token
有 Actions读写权限（用于触发转写工作流）。
