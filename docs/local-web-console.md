...
本地与 GitHub Pages 现在共用 `local_web/static/` 的深色界面：顶部导航提供课程、搜索、
订阅、自动化和设置。保留课程卡片分区下拉、置顶、模型管理、批量重跑和摘要版本对比；
Pages 原有的导出、删除、单次运行以及搜索范围筛选已整合到这套界面。

本地通过 Python API 读取资料、调用 GitHub 和管理 Vault。Pages 使用浏览器数据适配层，
不需要本地服务；Obsidian 同步和模型连接测试只在本地入口提供。

摘要使用内置安全 Markdown 排版；联网成功加载增强组件后，支持完整 Markdown 表格与
KaTeX 公式。增强组件不可用时仍可正常打开本地资料库。原始 HTML 经过转义或净化处理。

课程页提供整理区、学习区、查阅区和归档区。新课程默认在整理区；在卡片下拉中移动。
本地分区继续保存在 `course-zones.json`，Pages 分区保存在浏览器的仓库专属设置中。
两种入口共享界面代码，不会自动跨设备同步这些整理偏好。
...
- 本地登录表单将凭据提交到本机服务后清空；不写入浏览器存储，后续操作通过本机 API 完成。
...
每个课次会为不同的“供应商/模型”保留最新摘要版本。摘要页的“版本对比”可同时勾选
多个版本并排阅读；同一模型再次重跑会更新该模型自己的版本。课程详情页可以勾选多个
有转录的课次批量重跑，课程总览也可以勾选多个课程（会包含其中有转录的课次）。为控制
...
本地控制台支持数据库同步、课程/课次浏览、按课程及摘要/转录/OCR 范围分页搜索、订阅编辑、
单次运行、运行状态、手动检查、课程或课次的 PDF/Markdown/HTML 邮件导出与删除、模型管理、
单个或批量模型重跑、摘要版本对比，以及 Obsidian Markdown 同步。
