...
重跑、自动化和设置；页眉右侧固定两个图标按钮，所有页面保持一致——↻ 检查更新
...
显示，表格支持对齐并在窄屏上横向滚动。原始 HTML 不会执行；这样课程摘要即使包含意外的
HTML，也不能在本地页面运行脚本。阅读时选中文字即可点击浮出的“高亮”按钮做标记；
...
还没有 AI 标题的存量笔记，可在「自动化」页点「补齐笔记标题」批量生成——每篇一次小调用，
...
如需重跑，打开导航中的“重跑”页：勾选整门课程会重跑其中所有有转录的课次，展开课程
