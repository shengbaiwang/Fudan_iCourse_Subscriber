...
- **课程列表** — 所有已订阅课程，按最近更新排序，显示摘要数量与学期徽章；桌面端左侧、移动端抽屉中有 Notability 式侧栏（全部 / 星标 / 按学期 / 按院系，带计数、可折叠），选中即过滤主列表，选择与折叠状态经 localStorage 持久化（学期与院系取自 `all_courses` 目录中该课程最近的学期）
