...
保留日间/夜间主题、课程卡片上的分区下拉和相同的导航与操作。

| 能力 | 本地 | GitHub Pages |
| --- | --- | --- |
| 课程、课次、置顶、自定义分区、学期与院系筛选 | 支持 | 支持 |
| 摘要、转录、OCR、每次重跑的版本对比 | 支持 | 支持 |
| 多关键词、标题/内容筛选、分页搜索 | 支持 | 支持 |
| 订阅、单次运行、导出、删除、运行状态 | 支持 | 支持 |
| 模型配置、API Key 保存、单个和批量重跑 | 支持 | 支持 |
| 讲座录音上传、转写与笔记 | 支持 | 使用本地入口 |
...
安全设置。GitHub 拒绝批准时保留任务，界面显示原因，并将重试间隔延长至 5 分钟。
此功能代为完成批准步骤，不代表消除了 GitHub 平台触发审批的原因。
